 ...
 * ```
 */
export class StoreEmitter<T extends string = string> {
     ...
     */
    once(name: T, callback: Function, target: any) {
        let _self = this;
     ...
     * @param target - 触发函数作用域
     */
    listen(name: T, callback: Function, target: any) {
        if (!this._on[name]) {
            this._on[name] = [];
        }
     ...
     * @param target - 触发函数作用域
     */
    change(name: T, callback: Function, target: any) {
        this.listen(name, callback, target);
     ...
     * @param callback 设置函数
     */
    data(name: T, callback: (data: any) => any): any;
    data(name: T, value?: any): any {
     ...
     * @param target - 作用域
     */
    off(name: T, callback: Function, target: any) {
     ...
     * @param target - 作用域
     */
    remove(name: T, callback: Function, target: any) {
        const remove_list = [];
        ...
        for (let i = 0; i < eventList.length; i++) {
            ...
            const [cb, _target] = callEvent;
            if (cb === callback && _target === target) {
                remove_list.push(i);
            }
        }
